  ...
  <div class="account-address">
    <div class="gray-box clear">
      ...
      <div class="box-inner clear">
        <div class="address-list-item default-item" data-id="765532" v-for="address,index in receiveInfo">
          <div class="name fn-left">
            <div class="name-cell">{{address.name}}</div>
          </div>
          <div class="detail fn-left">
            <div class="detail-cell">{{address.province}} {{address.city}}{{address.county}}{{address.add}}</div>
          </div>
          <div class="operation fn-right">
            <div class="operation-cell">
              <a class="gray-edit-btn js-edit-address" @click="changePopHandle(index)">修改</a>
              <span class="red-del-btn js-del-address" @click="delReceiveInfoHandle(index)"> <a>删除</a><em>删除</em> </span>
            </div>
          </div>
          <div class="default fn-right" v-if="address.default">  （默认地址）  </div>
          <div class="setDefault fn-right" v-else="address.default" @click="defaultHandle(index)">
           <a>设为默认</a></div>
          <div class="telephone fn-right">{{address.phone}}</div>
        </div>
...
export default {
  ...
  methods: {
    defaultHandle (index) {
      this.$store.commit('defaultSetting', index)
    },
    closePop () {
      this.popShow = false
    ...
    showPop () {
      this.popShow = true
      this.$store.commit('clearBeforePop')
    },
    changePopHandle (index) {
      this.popShow = true
      this.$store.commit('changePop', index)
    },
    delReceiveInfoHandle (index) {
      this.$store.commit('delReceiveInfo', index)
    }
  }
}
</script>
